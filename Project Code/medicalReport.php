...
// 3. HANDLE FORM SUBMISSIONS (PHYSIO ONLY)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $is_physio) {
    ...
    if (isset($_POST['update_injury'])) {
        ...
        if (mysqli_query($conn, $update_q)) {
            ...
            // B. SYNC PLAYER TABLE (This is where the Player status updates!)
    ...
    if (isset($_POST['add_injury'])) {
        ...
        if (mysqli_query($conn, $insert_q)) {
            ...
            $p_status = 'Injured'; 
            if ($status == 'Doubtful') $p_status = 'Doubtful';
...
if ($is_physio) {
    ...
    while($row = mysqli_fetch_assoc($p_res)) { $player_list[] = $row; }
}
        ...
        <div class="content-wrapper fade-in">
            ...
            <div class="filter-container" style="margin-bottom: 25px; display: flex; gap: 20px; flex-wrap: wrap;">
                <div class="filter-group" style="flex: 1; min-width: 200px;">
                    <input type="text" id="searchInput" class="form-input" placeholder="Search by Player Name or Injury..." autocomplete="off">
                </div>
            ...
            <div class="injury-grid">
                <?php while ($row = mysqli_fetch_assoc($result)): 
                    ...
                    if (strpos($s, 'heal') !== false) $status_class = 'card-green';
                    elseif (strpos($s, 'critic') !== false || strpos($s, 'tear') !== false) $status_class = 'card-red';
                ?>
